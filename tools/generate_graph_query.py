...
def run_from_events(start, end):
    all_events = get_all_event_uri()
    for i in range(start, min(end, len(all_events))):
        test_query_json = generate_three_layer_graph(all_events[i], 3)
        if not test_query_json['graph']['edges']:
            continue
        # pprint(test_query_json)

        output_file = 'outouts/autogenerated_query_%d.xml' % i
        write_file(convert_query_json2xml(test_query_json, 'autogenerated_query_%d' % i), output_file)


run_from_events(1, 100)
