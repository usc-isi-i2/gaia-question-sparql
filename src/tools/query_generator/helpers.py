...
ENDPOINT = 'http://localhost:3030/run1dir00/'
...
def construct_ep(node, enttype, descriptors):
    ...
    for descriptor, obj in descriptors.items():
        ret[TYPED_DESCRIPTOR][descriptor] = obj[:max(1, len(obj)//4, 3)]
        ...
        for k, v in target.items():
            if isinstance(v, list):
                for v_ in v:
                    ...
                    update_xml(new_root, v_)
            else:
                new_root = ET.SubElement(root, k)
                update_xml(new_root, v)
